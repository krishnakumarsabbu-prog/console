...
export class FilesStore {
  ...
  async #init() {
    ...
    // Initial hydration from webcontainer snapshot (stub environments)
    ...
    try {
      if ((webcontainer as any)?.internal?.watchPaths) {
        ...
        // Fallback: no watcher in this environment
        const logger = createScopedLogger('FilesStore');
        logger.warn('File watching disabled: webcontainer internal.watchPaths unavailable');
      }
    } catch (error) {
      const logger = createScopedLogger('FilesStore');
      logger.error('Failed to initialize file watcher', error as any);
    }
    ...
    /**
     * In stub/web-only environments we may not get reliable watch events.
     * Periodically hydrate the file map from the in-memory webcontainer
     * snapshot so newly written files always appear in the workbench.
     */
    setInterval(() => {
      this.#hydrateFromSnapshot().catch((error) => {
        logger.error('Failed to periodically hydrate files from webcontainer snapshot', error);
      });
    }, 2000);
  }

  /**
   * Best-effort hydration: in stub environments expose all current files
   * from the in-memory webcontainer so the workbench has something to
   * display even if watcher events are missing or delayed.
   */
  async #hydrateFromSnapshot() {
    try {
      const webcontainer = await this.#webcontainer;
      const anyContainer = webcontainer as any;
      if (typeof anyContainer.listFiles !== 'function') {
        return;
      }

      const snapshot = anyContainer.listFiles() as { path: string; buffer: Uint8Array }[];
      const current = this.files.get();
      const updates: FileMap = { ...current };
      let fileCount = 0;

      for (const { path: filePath, buffer } of snapshot) {
        if (typeof filePath !== 'string') {
          continue;
        }

        // Only track files that live under the configured work dir
        ...
        const content = isBinary ? Buffer.from(buffer).toString('base64') : this.#decodeFileContent(buffer);

        updates[sanitizedPath] = {
          type: 'file',
          content,
          isBinary,
        };
      ...
      logger.error('Failed to hydrate files from webcontainer snapshot', error);
    }
  }
  ...
  #processEventBuffer(events: Array<[events: PathWatcherEvent[]]>) {
    ...
    for (const rawEvent of watchEvents) {
      const { type, buffer } = rawEvent as any;

      // Different environments may expose the path under different keys.
      const eventPath: string | undefined =
        (rawEvent as any).path ??
        (rawEvent as any).file ??
        (rawEvent as any).filename ??
        (rawEvent as any).name;

      // Safely skip events we can't associate with a path.
      if (!eventPath || typeof eventPath !== 'string') {
        logger.warn('Received file watcher event without path', rawEvent);
        continue;
      }
      ...
      switch (type) {
        ...
        case 'change': {
          ...
          const existingFile = currentFiles[sanitizedPath];
    ...
    if (Object.keys(updates).length > 0) {
      const newFiles = { ...currentFiles, ...updates };

      // Handle deletions (keys mapped to undefined) by actually removing them
  ...
  async deleteFolder(folderPath: string) {
    ...
    try {
      ...
      for (const [path, dirent] of Object.entries(newFiles)) {
...
function isBinaryFile(buffer: Uint8Array | undefined) {
  ...
  /**
   * Simple heuristic: try to decode as UTF-8 using the shared TextDecoder.
   * - If decoding throws, treat as binary.
   * - If it succeeds but the text contains a high ratio of control chars,
   *   also treat as binary.
   */
  try {
    const text = utf8TextDecoder.decode(buffer);

    // Count non-printable control characters (excluding common whitespace)
    let controlCount = 0;
    const len = Math.min(text.length, 1024); // sample up to first 1K chars

    for (let i = 0; i < len; i++) {
      const code = text.charCodeAt(i);

      if (
        (code >= 0 && code < 9) || // misc control
        (code > 13 && code < 32) || // more control chars
        code === 127
      ) {
        controlCount++;
      }
