...
export function useSettings(): UseSettingsReturn {
  const providers = useNanoStore(providersStore);
  const debug = useNanoStore(isDebugMode);
  const eventLogs = useNanoStore(isEventLogsEnabled);
  const promptId = useNanoStore(promptStore);
  const isLatestBranch = useNanoStore(latestBranchStore);
  const autoSelectTemplate = useNanoStore(autoSelectStarterTemplate);
  const [activeProviders, setActiveProviders] = useState<ProviderInfo[]>([]);
  const contextOptimizationEnabled = useNanoStore(enableContextOptimizationStore);
