...
const ProgressBar = ({ progress }: { progress: number }) => (
  <div className="w-full h-2 bg-bolt-elements-background-depth-3 rounded-full overflow-hidden">
    <motion.div
      className="h-full bg-bolt-elements-item-backgroundAccent"
      initial={{ width: 0 }}
    ...
    {progress.details && (
      <div className="mt-2 text-sm text-gray-600">
        {progress.details.changedFiles && progress.details.changedFiles.length > 0 && (
          <div className="mt-4">
            <div className="font-medium mb-2 text-bolt-elements-textPrimary">Changed Files:</div>
                  ...
                  <div key={type} className="space-y-1">
                    <div
                      className={classNames('text-sm font-medium', {
                        'text-bolt-elements-item-contentAccent': type === 'Modified',
                        'text-bolt-elements-item-contentSuccess': type === 'Added',
                        'text-bolt-elements-item-contentDanger': type === 'Deleted',
                      })}
                    >
                      ...
                      {filesOfType.map((file, index) => {
                        ...
                        return (
                          <div key={index} className="text-sm text-bolt-elements-textSecondary flex items-center gap-2">
                            <Icon
                              name={type === 'Modified' ? 'pencil' : type === 'Added' ? 'plus' : 'trash'}
        ...
        {progress.details.additions !== undefined && progress.details.deletions !== undefined && (
          <div className="mt-1">
            Changes: <span className="text-bolt-elements-item-contentSuccess">+{progress.details.additions}</span>{' '}
            <span className="text-bolt-elements-item-contentDanger">-{progress.details.deletions}</span>
          </div>
        )}
  ...
  return (
    <div className="flex flex-col gap-6">
      <motion.div
        ...
        transition={{ duration: 0.3 }}
      >
        <Icon name="arrow-up-circle" className="text-xl text-gray-500" />
        <div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={classNames(
                ...
                updateSettings.autoUpdate ? 'bg-bolt-elements-item-backgroundAccent' : 'bg-bolt-elements-background-depth-3',
              )}
            >
              <span
                className={classNames(
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={classNames(
                'relative inline-flex h-6 w-11 items-center rounded-full transition-colors',
                updateSettings.notifyInApp ? 'bg-bolt-elements-item-backgroundAccent' : 'bg-bolt-elements-background-depth-3',
              )}
            >
              <span
                className={classNames(
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <Icon name="refresh-cw" size={20} className="text-gray-500" />
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={classNames(
                'flex items-center gap-2 px-4 py-2 rounded-lg text-sm',
                'bg-bolt-elements-background-depth-3',
                'hover:bg-bolt-elements-item-backgroundAccent/10 hover:text-bolt-elements-item-contentAccent',
                'text-bolt-elements-textPrimary',
                'transition-colors duration-200',
        ...
        {error && (
          <div className="mt-4 p-4 bg-bolt-elements-item-backgroundDanger/10 text-bolt-elements-item-contentDanger rounded-lg border border-bolt-elements-item-backgroundDanger/20">
            {error}
          </div>
        )}
          ...
          <div className="mt-4 text-sm text-bolt-elements-textSecondary">
            ...
            {updateProgress?.details?.additions !== undefined && updateProgress?.details?.deletions !== undefined && (
              <div className="mt-2 flex items-center gap-2">
                <Icon name="git-branch" size={16} className="text-bolt-elements-textTertiary" />
                Changes: <span className="text-bolt-elements-item-contentSuccess">+{updateProgress.details.additions}</span>{' '}
                <span className="text-bolt-elements-item-contentDanger">-{updateProgress.details.deletions}</span>
              </div>
            )}
          </div>
        )}
        ...
        {updateProgress?.details?.changelog && (
          <div className="mb-6">
            ...
            <div className="bg-bolt-elements-background-depth-3 rounded-lg p-4 overflow-auto max-h-[300px]">
        ...
        {updateProgress?.details?.commitMessages && updateProgress.details.commitMessages.length > 0 && (
          <div className="mb-6">
            <p className="font-medium mb-2 text-bolt-elements-textPrimary">Changes in this Update:</p>
            <div className="bg-bolt-elements-background-depth-3 rounded-lg p-4 overflow-auto max-h-[400px]">
              <div className="prose dark:prose-invert prose-sm max-w-none">
        ...
        <Dialog>
          ...
          <DialogDescription>
            <div className="mt-4">
              ...
              {updateProgress?.details?.commitMessages && updateProgress.details.commitMessages.length > 0 && (
                <div className="mb-6">
                  <p className="font-medium mb-2 text-bolt-elements-textPrimary">Commit Messages:</p>
                  <div className="bg-bolt-elements-background-depth-3 rounded-lg p-3 space-y-2">
                    {updateProgress.details.commitMessages.map((msg, index) => (
                      <div key={index} className="text-sm text-bolt-elements-textSecondary flex items-start gap-2">
                        <Icon name="git-commit" size={16} className="text-bolt-elements-textTertiary mt-0.5 flex-shrink-0" />
                        <span>{msg}</span>
                      </div>
                    ))}
                  </div>
