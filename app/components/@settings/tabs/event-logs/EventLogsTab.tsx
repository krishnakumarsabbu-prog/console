...
const LogEntryItem = ({ log, isExpanded: forceExpanded, use24Hour, showTimestamp }: LogEntryItemProps) => {
  ...
  const style = useMemo(() => {
    if (log.category === 'provider') {
      return {
        icon: 'bot',
        color: 'text-bolt-elements-item-contentSuccess',
        bg: 'hover:bg-bolt-elements-item-backgroundSuccess/10',
        badge: 'text-bolt-elements-item-contentSuccess bg-bolt-elements-item-backgroundSuccess/10',
      };
    }
    ...
    if (log.category === 'api') {
      return {
        ...
        badge: 'text-bolt-elements-item-contentAccent bg-bolt-elements-item-backgroundAccent/10',
      };
    }
    ...
    switch (log.level) {
      case 'error':
        return {
          ...
          badge: 'text-bolt-elements-item-contentDanger bg-bolt-elements-item-backgroundDanger/10',
        };
      case 'warning':
        return {
          icon: 'alert-triangle',
          ...
          badge: 'text-bolt-elements-item-contentWarning bg-bolt-elements-item-backgroundWarning/10',
        };
      case 'debug':
        return {
          icon: 'bug',
  ...
  const renderDetails = (details: any) => {
        ...
        <div className="flex flex-col gap-2">
          ...
          {details.prompt && (
            <div className="flex flex-col gap-1">
              ...
              <pre className="text-xs text-bolt-elements-textSecondary bg-bolt-elements-background-depth-3 rounded p-2 whitespace-pre-wrap outline outline-1 outline-bolt-elements-borderColor/50">
                {details.prompt}
              </pre>
            </div>
          )}
    ...
    if (log.category === 'api') {
      return (
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-xs text-bolt-elements-textTertiary">
            <span className={details.method === 'GET' ? 'text-bolt-elements-item-contentSuccess' : 'text-bolt-elements-item-contentAccent'}>{details.method}</span>
            <span>•</span>
            <span>Status: {details.statusCode}</span>
            <span>•</span>
            <span>Duration: {details.duration}ms</span>
          </div>
          <div className="text-xs text-bolt-elements-textSecondary break-all font-mono opacity-80">{details.url}</div>
          {details.request && (
            <div className="flex flex-col gap-1">
          ...
          {details.error && (
            <div className="flex flex-col gap-1">
              <div className="text-xs font-medium text-bolt-elements-item-contentDanger">Error:</div>
              <pre className="text-xs text-bolt-elements-item-contentDanger bg-bolt-elements-item-backgroundDanger/10 rounded p-2 whitespace-pre-wrap outline outline-1 outline-bolt-elements-item-backgroundDanger/20">
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            {log.details && (
              <>
                <button
                  ...
                  className="text-xs text-bolt-elements-textTertiary hover:text-bolt-elements-item-contentAccent transition-colors text-left"
                >
            ...
            <div className="flex items-center gap-2 mt-1">
              <div className={classNames('px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider', style.badge)}>
                {log.level}
              </div>
              {log.category && (
                <div className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-bolt-elements-background-depth-3 text-bolt-elements-textTertiary border border-bolt-elements-borderColor/50">
                  {log.category}
                </div>
              )}
            </div>
        ...
        {showTimestamp && <time className="shrink-0 text-[10px] font-mono text-bolt-elements-textTertiary opacity-80">{timestamp}</time>}
      </div>
    </motion.div>
  );
};
  ...
  const ExportButton = () => {
    ...
    return (
      <DialogRoot open={isOpen} onOpenChange={handleOpenChange}>
        ...
        >
          <Icon name="search" size={18} className="text-gray-400 group-focus-within:text-gray-500 transition-colors" />
          Export
        </button>
      ...
      <div className="flex items-center justify-between">
        <DropdownMenu.Root open={showLevelFilter} onOpenChange={setShowLevelFilter}>
          <DropdownMenu.Trigger asChild>
            ...
            >
              <span
                className={classNames('text-lg', selectedLevelOption?.icon || 'funnel')}
                style={{ color: selectedLevelOption?.color }}
              />
              {selectedLevelOption?.label || 'All Types'}
              <Icon name="chevron-down" size={14} className={classNames('transition-transform duration-200', showLevelFilter ? 'rotate-180' : '')} />
            </button>
          ...
          <DropdownMenu.Portal>
            ...
            >
              {logLevelOptions.map((option) => (
                <DropdownMenu.Item
                  key={option.value}
                  className="group flex items-center px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-500/10 dark:hover:bg-gray-500/20 cursor-pointer transition-colors"
                  onClick={() => handleLevelFilterChange(option.value)}
                >
                  <div className="mr-3 flex h-5 w-5 items-center justify-center">
                    <div
                      className={classNames(option.icon, 'text-lg group-hover:text-gray-500 transition-colors')}
                      style={{ color: option.color }}
                    />
                  </div>
                  <span className="group-hover:text-gray-500 transition-colors">{option.label}</span>
                </DropdownMenu.Item>
              ))}
            </DropdownMenu.Content>
          </DropdownMenu.Portal>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <Icon name="refresh-cw" size={18} className={classNames(isRefreshing ? 'animate-spin' : '')} />
            Refresh
          </button>
      ...
      <div className="flex flex-col gap-4">
        <div className="relative">
          ...
          <div className="absolute left-3 top-1/2 -translate-y-1/2">
            <Icon name="search" className="text-lg text-gray-500 dark:text-gray-400" />
          </div>
        </div>
        ...
        {filteredLogs.length === 0 ? (
          ...
          >
            <Icon name="clipboard" className="text-4xl text-gray-400 dark:text-gray-600" />
            <div className="flex flex-col gap-1">
