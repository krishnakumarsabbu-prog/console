  ...
  const renderColorSection = () => (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        >
          <Icon name="refresh-cw" className="text-sm" />
          Reset
        </button>
      </div>
          ...
          >
            <div className="relative flex-shrink-0">
              ...
              <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-bolt-elements-bg-depth-1 rounded-full flex items-center justify-center shadow-sm">
                <Icon name="pencil" className="text-xs text-bolt-elements-textSecondary" />
              </div>
            </div>
            ...
            <div className="text-center space-y-2">
              ...
              {font.includes(f.key) && (
                <div className="w-6 h-6 mx-auto bg-bolt-elements-item-contentAccent rounded-full flex items-center justify-center">
                  <Icon name="check" className="text-bolt-elements-item-backgroundAccent text-sm" />
                </div>
              )}
            </div>
          </button>
          ...
          return (
            <div key={f.key} className="feature-card-container p-2">
              <button
                ...
                className={`group relative w-full p-6 text-sm font-medium transition-all duration-200 ${f.key === 'rounded'
                  ? isSelected
                    ? 'rounded-3xl'
                    : 'rounded-xl'
                  : f.key === 'border'
                    ...
                    ? isSelected
                      ? 'border-2 border-bolt-elements-borderColorActive bg-bolt-elements-item-backgroundAccent text-bolt-elements-item-contentAccent'
                  ...
                  } ${isSelected && f.key === 'shadow' ? 'shadow-xl' : 'shadow-md'}`}
                style={{
                  ...(f.key === 'gradient' && {
                    background: isSelected
                      ? 'linear-gradient(135deg, var(--bolt-elements-item-backgroundAccent) 0%, var(--bolt-elements-item-backgroundAccentHover) 100%)'
                      : 'var(--bolt-elements-bg-depth-3)',
                    color: isSelected ? 'var(--bolt-elements-item-contentAccent)' : 'var(--bolt-elements-textSecondary)',
                  }),
                }}
              >
                <div className="flex flex-col items-center gap-4">
                  <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-bolt-elements-bg-depth-1 bg-opacity-20">
                    ...
                    {f.key === 'border' && (
                      <div
                        className={`w-6 h-6 rounded-lg transition-all duration-200 ${isSelected ? 'border-2 border-current opacity-90' : 'border border-current opacity-70'
                          }`}
                      />
                    )}
                    {f.key === 'gradient' && (
                    ...
                    {f.key === 'frosted-glass' && (
                      <div className="relative">
                        className={`w-6 h-6 rounded-lg transition-all duration-200 backdrop-blur-sm bg-white/10 border border-white/20 ${isSelected ? 'opacity-90' : 'opacity-70'
                          }`}
                        <div
