...
function InlineInput({ depth, placeholder, initialValue = '', onSubmit, onCancel }: InlineInputProps) {
  ...
  return (
    ...
    >
      <Icon name="file-plus" className="scale-120 shrink-0 text-bolt-elements-textTertiary" />
      <input
        ref={inputRef}
        type="text"
...
function Folder({ folder, collapsed, selected = false, onCopyPath, onCopyRelativePath, onClick }: FolderProps) {
  ...
  return (
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={folder.fullPath}>
      <NodeButton
        className={classNames('group transition-all duration-200', {
          'bg-transparent text-[var(--vscode-text)] hover:bg-[var(--vscode-hover)]':
            !selected,
          'bg-[var(--vscode-selected)] text-[var(--vscode-text)]': selected,
        })}
        depth={folder.depth}
        icon={collapsed ? 'caret-right' : 'caret-down'}
        iconClassName="scale-98"
        onClick={onClick}
      >
        <div className="flex items-center w-full">
          ...
          {isLocked && (
            <Icon
              name="lock-simple"
              className="shrink-0 scale-80 text-red-500"
...
function File({
  ...
  return (
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={fullPath}>
      <NodeButton
        className={classNames('group transition-all duration-200', {
          'bg-transparent hover:bg-[var(--vscode-hover)] text-[var(--vscode-text)]':
            !selected,
          'bg-[var(--vscode-selected)] text-[var(--vscode-text)]': selected,
        })}
        depth={depth}
        icon="file"
        iconClassName={classNames('scale-98', {
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-1">
            ...
            {unsavedChanges && <Icon name="circle-fill" className="scale-68 shrink-0 text-orange-500" />}
          </div>
        </div>
      </NodeButton>
    </FileContextMenu>
...
function NodeButton({ depth, icon, iconClassName, onClick, className, children }: ButtonProps) {
  return (
    <button
      className={classNames(
        'flex items-center gap-1.5 w-full pr-2 text-faded px-2 py-1.5 rounded-md',
        className,
      )}
      style={{ paddingLeft: `${6 + depth * NODE_PADDING_LEFT}px` }}
      onClick={() => onClick?.()}
    >
      <Icon name={icon} className={classNames('shrink-0', iconClassName)} />
