...
export function LockManager() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-[var(--vscode-sidebar-bg)]">
      {/* Controls */}
      <div className="flex items-center gap-1 px-2 py-1">
        {/* Search Input */}
        <div className="relative flex-1">
          <Icon name="search" size={12} className="absolute left-2 top-1/2 -translate-y-1/2 text-bolt-elements-textTertiary pointer-events-none" />
          <input
            type="text"
            placeholder="Search..."
            className="w-full text-xs pl-6 pr-2 py-0.5 h-6 bg-[var(--vscode-bg)] text-[var(--vscode-text)] rounded border border-[var(--vscode-border)] focus:outline-none focus:ring-1 focus:ring-[var(--vscode-active-border)]"
            value={searchTerm}
        ...
        {/* Filter Select */}
        <select
          className="text-xs px-1 py-0.5 h-6 bg-transparent text-bolt-elements-textPrimary rounded border border-white/10 focus:outline-none focus:ring-2 focus:ring-white/20"
          value={filter}
      ...
      <div className="flex-1 overflow-auto modern-scrollbar px-1 py-1">
        {filteredAndSortedItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-bolt-elements-textTertiary text-xs gap-2">
            <Icon name="lock-open" className="text-lg opacity-50" />
          ...
          <ul className="space-y-1">
            {filteredAndSortedItems.map((item) => (
              ...
              >
                ...
                />
                <Icon
                  name={item.type === 'file' ? 'file-text' : 'folder'}
                  className="shrink-0 text-bolt-elements-textTertiary text-xs"
                />
                ...
                <button
                  ...
                  title="Unlock"
                >
                  <Icon name="lock-open" className="text-xs" />
                </button>
              </li>
            ))}
          </ul>
