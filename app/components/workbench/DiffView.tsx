...
const renderContentWarning = (type: 'binary' | 'error') => (
  <div className="h-full flex items-center justify-center p-4">
    <div className="text-center text-bolt-elements-textTertiary">
      <Icon name={type === 'binary' ? 'x-circle' : 'alert-triangle'} className="text-4xl text-red-400 mb-2 mx-auto" />
...
const NoChangesView = memo(
  ({
    ...
    <div className="h-full flex flex-col items-center justify-center p-4">
      <div className="text-center text-bolt-elements-textTertiary">
        <Icon name="files" className="text-4xl text-green-400 mb-2 mx-auto" />
  ...
  ({
    ...
    return (
      <div className="flex items-center bg-bolt-elements-background-depth-1 p-2 text-sm text-bolt-elements-textPrimary shrink-0">
        <Icon name="file" className="mr-2 h-4 w-4 shrink-0" />
...
export const DiffView = memo(({ fileHistory, setFileHistory }: DiffViewProps) => {
  const files = useNanoStore(workbenchStore.files) as FileMap;
  const selectedFile = useNanoStore(workbenchStore.selectedFile);
  const currentDocument = useNanoStore(workbenchStore.currentDocument) as EditorDocument;
    ...
    return (
      <div className="flex w-full h-full justify-center items-center bg-bolt-elements-background-depth-1 text-red-400">
        <div className="text-center">
          <Icon name="alert-triangle" className="text-4xl mb-2" />
...
function getHighlighter(arg0: { themes: string[]; langs: string[]; }): Promise<any> | null {
